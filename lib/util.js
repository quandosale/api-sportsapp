...
module.exports = {
    ...
    saveEcgToFile: (res, value, owner, datetime, duration, bAppend) => {
        ...
        var ecg_headerfile = `${filename}_ecg.hea`;
        var accel_signalfile = `${filename}_accel.dat`;
        var accel_headerfile = `${filename}_accel.hea`;
        var hr_file = `${filename}_hr.dat`;
        var recordfile = `${ownerId}${new Date(datetime).getTime()}`;
        var af_file = `${filename}_af.dat`;

        const ecg_buf = ecgToBuffer(value.ecg);
        const accel_buf = accelToBuffer(value);
        ...
        if (!bAppend) {
            ...
            var sampleFreq = sampleCount / duration;
            var headerData = `${recordfile}_ecg 1 ${sampleFreq} ${sampleCount}\n${recordfile}_ecg.dat 16`;
            fs.writeFile(ecg_headerfile, headerData);

            // fs.writeFileSync(accel_signalfile, accel_buf);
            var headerData = `${recordfile}_accel 3 ${sampleFreq} ${sampleCount}\n${recordfile}_accel.dat 16\n${recordfile}_accel.dat 16\n${recordfile}_accel.dat 16`;
            fs.writeFile(accel_headerfile, headerData);
            ...
            fs.appendFileSync(accel_signalfile, accel_buf);
            var headerData = `${recordfile}_accel 3 ${sampleFreq} ${sampleCount}\n${recordfile}_accel.dat 16\n${recordfile}_accel.dat 16\n${recordfile}_accel.dat 16`;
            fs.writeFile(accel_headerfile, headerData);
